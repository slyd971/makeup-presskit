...
export function Hero({ artist }: HeroProps) {
  ...
  const renderStats = () => (
    <>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {artist.collaborations.slice(0, 6).map((collaboration, index) => (
          <FadeIn key={collaboration} delay={0.2 + index * 0.04}>
            <div className="rounded-full border border-black/8 bg-white/72 px-3 py-1.5 text-[10px] uppercase tracking-[0.18em] text-ink/58 shadow-soft backdrop-blur">
              {collaboration}
            </div>
          </FadeIn>
        ))}
  ...
  const renderTextBlock = (compact = false) => (
    <FadeIn className={compact ? "max-w-xl" : "max-w-2xl"}>
      <p className="text-sm uppercase tracking-[0.35em] text-ink/45">
        {artist.location}
      </p>
  ...
  if (artist.hero.variant === "portrait-stack") {
    return (
      <section id="home" className="relative overflow-hidden px-6 pb-5 pt-24 md:px-10 md:pb-7 md:pt-28">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-7 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div className="order-1 grid gap-5 md:grid-cols-[0.58fr_0.42fr] lg:order-2">
            <FadeIn delay={0.08}>
              <div className="relative overflow-hidden rounded-[2rem] bg-white p-3 shadow-glow">
                <div className="relative aspect-[4/5] overflow-hidden rounded-[1.5rem] md:aspect-[4/4.55]">
                  <Image
              ...
              {artist.hero.secondaryImage ? (
                <FadeIn delay={0.22}>
                  <div className="relative overflow-hidden rounded-[1.75rem] bg-white p-3 shadow-soft">
                    <div className="relative aspect-[3/4] overflow-hidden rounded-[1.25rem] md:aspect-[3/3.6]">
                      <Image
  ...
  return (
    <section id="home" className="relative overflow-hidden px-6 pb-5 pt-24 md:px-10 md:pb-7 md:pt-28">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-7 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
        <FadeIn delay={0.12} className="order-1 lg:order-2">
          <div className="relative mx-auto max-w-xl">
            <div className="absolute -inset-6 rounded-[2rem] blur-3xl" style={frameGlowStyle} />
            <div className="relative overflow-hidden rounded-[2rem] bg-white p-3 shadow-glow">
              <div className="relative aspect-[4/5] overflow-hidden rounded-[1.5rem] md:aspect-[4/4.5]">
                <Image
                  src={artist.hero.primaryImage}
                  alt={artist.hero.primaryAlt}
                  fill
